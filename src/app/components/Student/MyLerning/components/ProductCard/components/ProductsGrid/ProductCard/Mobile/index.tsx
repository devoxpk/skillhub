...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const createLinkedItem = (type: string, config: IConfig) => {
    console.log(type);
    return (
      <div className="linked-item flex items-center gap-2 cursor-pointer py-0.5 px-1 mx-[-4px] my-[-2px] rounded-sm transition-colors duration-200 ease-in-out group text-[11px]">
        ...
        <div className="flex items-center gap-2 text-[#65676b] flex-1">
          <span className="text-[#65676b]">Linked to </span>
          <div className="flex ites-center gap-1">
            ...
            {config.showCount !== false &&
              ...
              config.total > config.items.length && (
                <span className="text-[#65676b] relative pl-1 group-hover:text-[#13c4cc] group-hover:underline">{`${config.total} ${config.itemType}`}</span>
              )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col border border-[#e0e0e0] rounded-2xl overflow-hidden transition-border-color duration-200 hover:border-[#ccc] w-full">
      <div className="flex flex-col">
        <Image
          className="object-cover rounded-t-2xl mr-4 w-full h-[160px]"
          src={image}
          alt={title}
          width={300}
          height={160}
        ...
        <div className="flex flex-col p-4">
          <div className="flex justify-between items-center">
            ...
            <button className="ml-3 w-8 h-8 border border-[#e0e0e0] rounded-md flex items-center justify-center cursor-pointer transition-colors duration-200 hover:bg-[#f5f5f5]">
          ...
          <h3 className="text-md font-semibold text-[#333] my-2 tracking-[-0.011em] leading-6">
            {title}
          </h3>
          <div className="flex items-center gap-2">
      ...
      {[
        ...
        <div className="bg-[#f8f9fa] broder-t border-[#e0e0e0] px-3 py-2 flex flex-col gap-2 rounded-b-lg">
          <div className="flex flex-col gap-2">{createCardFooter(type)}</div>
        </div>
      )}
    </div>
  );
