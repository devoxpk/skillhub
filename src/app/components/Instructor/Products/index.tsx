...
const ProductsInstructor = () => {
  ...
  const {currentPage, filteredProducts, setCurrentPage, setFilteredProducts} = useProduct();
  ...
  useEffect(() => {
    ...
    if (typeFilters.length > 0) {
      filtered = filtered.filter((product) => {
        ...
        return productType && typeFilters.some(filter => filter.toLowerCase() === productType);
      });
    }
  ...
  if (isMobile) {
    return (
      <div>
        ...
        <div className="xs:bg-white rounded-xl max-xs:p-2  max-xs:pt-0 p-6 shadow-sm">
            ...
            <div className="flex items-center gap-6">
              <div className="text-[#666] text-sm">
                ...
                <strong className="text-[#333] font-semibold">
                  {filteredProducts.length=== 0 ? indexOfFirstProduct: indexOfFirstProduct +1}-
          ...
          <div className="flex relative items-center gap-2">
            <h1 className="text-3xl font-semibold leading-8 tracking-wide text-[#333] max-xs:text-2xl">
                {typeFilters.length === 0 
                  ? "All Products"
                  : (() => {
                      const validTypes = typeFilters.filter(type => type in typeMap);
                      if (validTypes.length === 0) return "All Products";
                      
                      const mappedTypes = validTypes.map(type => typeMap[type as keyof typeof typeMap]);
                      if (validTypes.length === 1) return mappedTypes[0];
                      if (validTypes.length === 2) return `${mappedTypes[0]}, ${mappedTypes[1]}`;
                      return `${mappedTypes[0]}, ${mappedTypes[1]} +${validTypes.length - 2}`;
                    })()
                }
            </h1>
            <button
